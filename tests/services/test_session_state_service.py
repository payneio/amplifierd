...
class TestSessionStateService:
    ...
    def test_session_metadata_cwd_property_project_directory(
        self,
        sample_mount_plan: MountPlan,
    ) -> None:
        """Test CWD is computed correctly from session_cwd for project directory."""
        metadata = SessionMetadata(
            session_id="test-123",
            amplified_dir="projects/test-project",
            session_cwd="projects/test-project",  # Starts same as amplified_dir
            profile_name="test/profile",
        ...
        # Verify CWD is computed correctly
        config = load_config()
        data_path = Path(config.data_path)
        expected_cwd = (data_path / "projects/test-project").resolve()
        assert metadata.cwd == expected_cwd
        assert metadata.cwd.is_absolute()

    def test_session_metadata_cwd_property_root_directory(
        self,
        sample_mount_plan: MountPlan,
    ) -> None:
        """Test CWD for root amplified directory ('.')."""
        metadata = SessionMetadata(
        ...
        # Verify CWD is the data root
        config = load_config()
        data_path = Path(config.data_path)
        expected_cwd = data_path.resolve()
        assert metadata.cwd == expected_cwd

    def test_session_metadata_cwd_property_nested_directory(
        self,
        sample_mount_plan: MountPlan,
    ) -> None:
        """Test CWD for deeply nested directory."""
        metadata = SessionMetadata(
            session_id="test-nested",
            amplified_dir="projects/sub1/sub2/sub3",
            session_cwd="projects/sub1/sub2/sub3",  # Starts same as amplified_dir
            profile_name="test/profile",
            status=SessionStatus.CREATED,
            created_at=datetime.now(UTC),
            mount_plan_path="mount_plan.json",
        ...
        config = load_config()
        data_path = Path(config.data_path)
        expected_cwd = (data_path / "projects/sub1/sub2/sub3").resolve()
        assert metadata.cwd == expected_cwd

    def test_get_session_cwd_method(
        self,
        service: SessionStateService,
        sample_mount_plan: MountPlan,
    ) -> None:
        """Test get_session_cwd() method returns correct CWD."""
        # Create session with amplified_dir
        service.create_session(
            session_id="test-cwd-123",
            profile_name="test/profile",
            mount_plan=sample_mount_plan,
            amplified_dir="projects/test",
        )

        # Get CWD using service method
        cwd = service.get_session_cwd("test-cwd-123")

        # Verify it matches expected path
        config = load_config()
        data_path = Path(config.data_path)
        expected = (data_path / "projects/test").resolve()
        assert cwd == expected
        assert cwd.is_absolute()

    def test_get_session_cwd_not_found(
        self,
        service: SessionStateService,
    ) -> None:
        """Test get_session_cwd raises ValueError for non-existent session."""
        with pytest.raises(ValueError, match="Cannot get CWD.*session not found"):
            service.get_session_cwd("nonexistent-session-id")

    def test_get_session_cwd_after_session_creation(
        self,
        service: SessionStateService,
        sample_mount_plan: MountPlan,
    ) -> None:
        """Test CWD is immediately available after session creation."""
        metadata = service.create_session(
            session_id="test-immediate-cwd",
            profile_name="test/profile",
            mount_plan=sample_mount_plan,
            amplified_dir="projects/immediate",
        )

        # CWD should be accessible via property
        config = load_config()
        data_path = Path(config.data_path)
        expected = (data_path / "projects/immediate").resolve()
        assert metadata.cwd == expected

        # CWD should also be accessible via service method
        cwd = service.get_session_cwd("test-immediate-cwd")
        assert cwd == expected

    def test_cwd_remains_consistent_across_state_transitions(
        self,
        service: SessionStateService,
        sample_mount_plan: MountPlan,
    ) -> None:
        """Test CWD doesn't change during session lifecycle (unless explicitly changed)."""
        # Create session
        metadata_created = service.create_session(
            session_id="test-consistent-cwd",
            profile_name="test/profile",
            mount_plan=sample_mount_plan,
            amplified_dir="projects/consistent",
        )
        cwd_created = metadata_created.cwd

        # Start session
        service.start_session("test-consistent-cwd")
        metadata_active = service.get_session("test-consistent-cwd")
        assert metadata_active is not None
        cwd_active = metadata_active.cwd

        # Complete session
        service.complete_session("test-consistent-cwd")
        metadata_completed = service.get_session("test-consistent-cwd")
        assert metadata_completed is not None
        cwd_completed = metadata_completed.cwd

        # Verify CWD is the same throughout lifecycle
        assert cwd_created == cwd_active == cwd_completed

    def test_session_cwd_can_differ_from_amplified_dir(
        self,
        sample_mount_plan: MountPlan,
    ) -> None:
        """Test that session_cwd and amplified_dir serve different purposes."""
        # Create metadata where session_cwd differs from amplified_dir
        # (simulating a cd command changing the working directory)
        metadata = SessionMetadata(
            session_id="test-cwd-diff",
            amplified_dir="projects/test-project",  # Where .amplified/ config lives
            session_cwd="projects/test-project/src",  # Where tools currently work
            profile_name="test/profile",
            status=SessionStatus.CREATED,
            created_at=datetime.now(UTC),
            mount_plan_path="mount_plan.json",
        ...
        # Verify amplified_dir is preserved (for loading .amplified/ config)
        assert metadata.amplified_dir == "projects/test-project"

        # Verify cwd reflects current working directory
        config = load_config()
        data_path = Path(config.data_path)
        expected_cwd = (data_path / "projects/test-project/src").resolve()
        assert metadata.cwd == expected_cwd

        # They're different - this is correct behavior
        assert metadata.session_cwd != metadata.amplified_dir
